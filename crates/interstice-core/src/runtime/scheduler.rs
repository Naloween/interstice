...
mod tests {
    ...
    fn insert_insert_same_table_can_run_together() {
        ...
        let first = s.enqueue("a", vec![acc("m", "t", TableOp::Insert)]);
        let first = first.into_iter().collect::<Vec<_>>();
        assert_eq!(first.len(), 1);
        let second = s.enqueue("b", vec![acc("m", "t", TableOp::Insert)]);
    ...
    fn waiting_queue_allows_later_non_conflicting_jobs() {
        let mut s = ReducerScheduler::new();
        let running = s
            .enqueue("run", vec![acc("m", "a", TableOp::Update)])
            .into_iter()
            .collect::<Vec<_>>();
        assert_eq!(running.len(), 1);

        // Both wait behind `run`'s update on `a`; reads do not conflict with each other once `run` completes.
        s.enqueue("wait1", vec![acc("m", "a", TableOp::Read)]);
        s.enqueue("wait2", vec![acc("m", "a", TableOp::Read)]);
    ...
    fn read_conflicts_with_insert_same_table() {
        let mut s = ReducerScheduler::new();
        let first = s
            .enqueue("reader", vec![acc("m", "t", TableOp::Read)])
            .into_iter()
            .collect::<Vec<_>>();
        assert_eq!(first.len(), 1);

        s.enqueue("writer", vec![acc("m", "t", TableOp::Insert)]);
