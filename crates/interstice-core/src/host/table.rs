...
impl Runtime {
    pub fn handle_insert_row(&mut self, insert_row_request: InsertRowRequest) -> InsertRowResponse {
        let reducer_frame = self.call_stack.last_mut().unwrap();
        reducer_frame.transaction.inserts.push(insert_row_request);
        InsertRowResponse {}
    }
    pub fn handle_update_row(&mut self, update_row_request: UpdateRowRequest) -> UpdateRowResponse {
        let reducer_frame = self.call_stack.last_mut().unwrap();
        reducer_frame.transaction.updates.push(update_row_request);
        UpdateRowResponse {}
    }
    pub fn handle_delete_row(&mut self, delete_row_request: DeleteRowRequest) -> DeleteRowResponse {
        let reducer_frame = self.call_stack.last_mut().unwrap();
        reducer_frame.transaction.deletes.push(delete_row_request);
        DeleteRowResponse {}
    }
    pub fn handle_table_scan(&mut self, table_scan_request: TableScanRequest) -> TableScanResponse {
        self.modules
