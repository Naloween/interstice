...
pub enum CompatibilityChange {
    ...
    FieldTypeChanged { table: String, field: String, from: String, to: String },
}
...
pub fn inspect_log(log_path: &Path) -> Result<LogInspectionResult, String> {
    ...
    let validator_result =
        LogValidator::validate(log_path).map_err(|e| format!("Failed to validate log: {}", e))?;

    let info =
        LogValidator::inspect(log_path).map_err(|e| format!("Failed to inspect log: {}", e))?;
...
pub fn diff_schemas(old_path: &Path, new_path: &Path) -> Result<SchemaDiffResult, String> {
    ...
    for old_reducer in &old.reducers {
        if !new.reducers.iter().any(|r| r.name == old_reducer.name) {
            breaking_changes.push(CompatibilityChange::ReducerRemoved(old_reducer.name.clone()));
        }
    }
