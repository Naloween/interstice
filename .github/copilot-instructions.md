...
- The CLI uses top-level subcommands (`node`, `bindings`, `example`, `init`, `load`, `unload`, `remove`, `call_reducer`, `call_query`); entry in [crates/interstice-cli/src/main.rs](crates/interstice-cli/src/main.rs).
...
- Load/unload/remove module on a running node:
  - `cargo run -p interstice-cli -- load <node> <module_project_path>`
  - `cargo run -p interstice-cli -- unload <node> <module_name>` (keeps the module's persisted data; marks it unloaded via `state.toml` so node startup skips it)
  - `cargo run -p interstice-cli -- remove <node> <module_name>` (deletes the module's persisted data)
